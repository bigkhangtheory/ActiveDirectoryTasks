<#
    ...
    .PARAMETER Paths
        List of Active Directory paths to the target Configuration object.
#>
#Requires -Module ActiveDirectoryDsc


configuration ConfigurationContainerAcls
{
    param
    (
        [Parameter(Mandatory)]
    ...
    <#
        This function is used to resolve Schema GUIDs
    #>
    ...
    function Get-ObjectPathSplat
    {
        ...
        # set OU path as DN
        $objectPath = 'CN={0},{1}' -f $Object.Name, $ParentPath
        ...
        if ($Object.AccessControlList)
        {
            ...
            foreach ($a in $Object.AccessControlList)
            {
                ...
                foreach ($e in $a.PermissionEntries)
                {
                    ...
                    $executionName = "$($objectPath -replace '[,|=\s]', '')_$($e.IdentityReference -replace '[-().:\s]', '_')_$($e.AccessControlType)_$($e.ActiveDirectoryRights | Select-Object -First 1)_$($e.ObjectType -replace '[-().:\s]', '_')_$($index)"

                    Write-Verbose "[$($Object.Name)] - Execution name is $executionName"
...
"@
    ...
    foreach ($p in $Paths)
    {
        ...
        if ($p.Path -notmatch '(?<DomainPart>dc=\w+,dc=\w+)')
        {
            $p.Path = "$($p.Path),$DomainDN"
        }
